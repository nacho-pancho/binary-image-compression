...
//	  std::cout << i2 << ' ' << j2 << std::endl;
	...
	P3 = P.get_copy();
      ...
      const idx_t nomatch_nonpred_len = 1 + 1 + enumL(M,nomatch_nonpred_weight);
      const idx_t nomatch_pred_len = 1 + 1 + enumL(M,nomatch_pred_weight);
      const idx_t match_nonpred_len = 1 + 1 + idx_len + enumL(M,match_nonpred_weight);
      const idx_t match_pred_len = 1 + 1 + idx_len + enumL(M,match_pred_weight);
